...
# Calculate area of CCC from shapefile (area calculated using UTM 34N projection)
ccc = read.dbf("E:/research/2018_12_20_Borges_etal_Congo/GIS/output/ccc/glc2009_ccc_v1.dbf")
ccc_km2 = sum(ccc$area)
...
# only hSHEDS with stream order > 3, GRWL widths > 0, non lakes, & in the CCC:
...
barplot(width_byOrd_obs, names.arg=uniqOrd, 
        main="CCC Median Observed River Width by Order",
        xlab="Order",
...
plot(range(c(modOrds, uniqOrd)), range(c(width_byOrd, modWidth_byOrd)), type='n',
     main="CCC Median Width by Order",
     xlab="Order",
     ylab="Median Width (m)")
xSeq = seq(-1, max(uniqOrd)+1, length.out=100)
ySeq = exp(logW_lm$coefficients[[2]]*xSeq+logW_lm$coefficients[[1]])
lines(xSeq, ySeq, col="dark green", lty=1)
points(modOrds, modW, pch=17, col="dark green", cex=1.4)
points(uniqOrd, width_byOrd_obs, pch=16, col=1)


#### 
# store the median width of order 9 to be added into the RSSA calc later:
ord9width = width_byOrd_obs[uniqOrd == 9]

# exclude order 9 when fitting the model because it throws off the model substantially:
uniqOrd = uniqOrd[uniqOrd != 9]
...
modOrds = c(1:c(min(uniqOrd)-1))
modW = exp(logW_lm$coefficients[[2]]*modOrds+logW_lm$coefficients[[1]])
width_byOrd = c(modW, width_byOrd_obs, ord9width) #add back in the 9th order width:
...
legend("topleft", c("Observed", "Modeled with order 9", "Modeled without order 9"), 
       col=c(1,"dark green",2), pch=c(16,17, 16), inset=0.05)
...
# remove order 10 since it has a length of 0:
uniqOrd = uniqOrd[uniqOrd != 10]
...
plot(c(modOrds, uniqOrd), len_byOrd, type='n',
     main="CCC Median Length by Order",
     xlab="Order",
...
barplot(RSSA_byOrd, names.arg=modOrds_all, 
        main="CCC River & Stream Surface Area",
        xlab="Order",
...
write.csv(RSSA_byOrd_tab, "E:/research/2018_12_20_Borges_etal_Congo/tabs/RSSA_byOrder_ccc.csv", row.names=F)

# print basin %RSSA:
print(paste0(round(100*sum(RSSA_byOrd)/ccc_km2, 2), "% RSSA in the CCC"))
...
plot(c(modOrds, uniqOrd), slope_byOrd, type='n',
     main="CCC Median River Slope by Order",
     xlab="Order",
...
plot(c(modOrdV, uniqOrd), vel_byOrd, type='n',
     main="CCC Median River Velocity by Order",
     xlab="Order",
...
barplot(q_byOrd_obs, names.arg=uniqOrd, 
        main="CCC Median River Discharge",
        xlab="Order",
...
plot(c(modOrds, uniqOrd), q_byOrd, type='n',
     main="CCC Median River Discharge by Order",
     xlab="Order",
...
write.csv(ord_tab, "E:/research/2018_12_20_Borges_etal_Congo/tabs/ord_tab_CCC.csv", row.names=F)
